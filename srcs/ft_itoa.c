...
int len_base(char c, __uintmax_t nb, int *base)
{
	int len;
...
int ft_itoa(t_param param, __intmax_t nb, char sign_flag)
{
	...
	param.width == 0 && sign_flag == ' ' && nb > 0 ? a.length++ : 0;
...
void ft_uitoa_1(t_for_itoa *a, t_param param)
{
	...
	if ((a->flag == 2 && a->buf) || (a->flag && param.conversion == 'p'))
	{
...
void ft_uitoa_3(t_for_itoa *a, t_param param)
{
	a->j = a->length - 5;
	a->i = 0;
	while (a->i < a->j)
...
int ft_uitoa(t_param param, __uintmax_t nb)
{
	...
	if (param.conversion == 'p' && !nb)
	{
		param.precision = -1;
		a.flag = 0;
		a.count = 5;
	}
	param.conversion == 'u' && !param.precision ? a.count = 0 : 0;
	a.length = a.count;
	a.length < param.precision ? a.length = param.precision : 0;
	...
	if  (param.conversion == 'p' && !nb)
			ft_uitoa_3(&a, param);
	else if (param.flags & FLAG_MINUS)
		ft_uitoa_1(&a, param);
	else
		ft_uitoa_2(&a, param);
	return (a.k);
